  ...
  const pendingOrders: Order[] = [
    {
      ...
      description: "Ortofotomapa działki budowlanej o powierzchni 2 ha z dokładnością 2cm/px.",
    ...
    },
    {
      ...
      service: "Modele 3D",
      description: "Model 3D budynku mieszkalnego o wysokiej dokładności z teksturami.",
    ...
    },
    {
      ...
      description: "Skanowanie laserowe terenu przemysłowego z gęstością 100 pkt/m².",
...
async function fetch_completed_orders(): Promise<Order[]> {
  ...
  const completedOrders: Order[] = [
    ...
    },
    {
      ...
      service: "Inspekcja",
      description: "Szczegółowa inspekcja stanu dachu z dokumentacją fotograficzną.",
...
async function rate_order(ratingData: RatingData): Promise<{ success: boolean; message?: string }> {
  await mockDelay(API_DELAY);
  
  if (!ratingData.orderId || !ratingData.rating || ratingData.rating < 1 || ratingData.rating > 5) {
    return {
      success: false,
...
async function select_operator(orderId: number, operatorId: number): Promise<{ success: boolean; message?: string }> {
  await mockDelay(API_DELAY);
